...
function App() {
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:5050/api/user?action=${loginMode ? 'login' : 'signup'}`, {
        method: 'POST',
  ...
  const uploadData = async () => {
    try {
      const res = await fetch('http://localhost:5050/api/private/upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('http://localhost:5050/api/private/fetch', {
        method: 'GET',
        credentials: 'include',
      });
